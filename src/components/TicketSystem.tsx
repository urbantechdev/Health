...
export default function TicketSystem() {
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 max-w-7xl mx-auto animate-fade-in relative">
      ...
      {/* SUCCESS PROMPT CENTERED MODAL / WINDOW */}
      <AnimatePresence>
        {successPrompt?.show && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/75 backdrop-blur-md">
            <motion.div
              initial={{ opacity: 0, scale: 0.88, y: 25, filter: "blur(6px)" }}
              animate={{ opacity: 1, scale: 1, y: 0, filter: "blur(0px)" }}
              exit={{ opacity: 0, scale: 0.94, y: 15, filter: "blur(4px)" }}
              transition={{ type: "spring", stiffness: 420, damping: 28 }}
              className="relative bg-slate-950 text-white w-full max-w-lg rounded-3xl shadow-2xl border border-emerald-500/30 overflow-hidden flex flex-col p-6 sm:p-7 gap-5"
              style={{
                boxShadow: "0 30px 70px -15px rgba(0,0,0,0.8), 0 0 50px -5px rgba(16, 185, 129, 0.35)",
              }}
            >
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-4">
                  ...
                  <AnimatedSuccessTick size="md" />

                  <div className="space-y-1">
                    <span className="inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-wider px-2.5 py-0.5 rounded-full border bg-emerald-500/15 text-emerald-300 border-emerald-500/30">
                ...
                <button
                  ...
                  className="p-2 text-slate-400 hover:text-white hover:bg-slate-800 rounded-2xl transition-colors shrink-0 cursor-pointer"
                  title="Close Window"
                >
                  <X className="w-5 h-5" />
                </button>
                ...
                {successPrompt.ticketNumber && (
                  <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-4 space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Queue Ticket Identifier</span>
                      <button
                        type="button"
                        onClick={() => copyToClipboard(successPrompt.ticketNumber!)}
                        className="text-xs font-bold text-emerald-400 hover:text-emerald-300 bg-emerald-950/60 border border-emerald-800/80 px-3 py-1 rounded-xl transition-all flex items-center gap-1.5 cursor-pointer shadow-xs active:scale-95"
                      >
                        {copiedTicketNo ? (
                          <>
                          ...
                          <>
                            ...
                            <span>Copy Ticket No</span>
                          </>
                        )}
                      </button>
                    </div>

                    <div className="flex items-center justify-center p-3.5 bg-slate-950 rounded-xl border border-slate-800">
                      <span className="text-2xl sm:text-3xl font-black font-mono tracking-widest text-emerald-400">
                        {successPrompt.ticketNumber}
                      </span>
                    </div>

                    {successPrompt.department && (
                      <div className="flex items-center justify-between text-xs text-slate-400 pt-1 border-t border-slate-800">
                        <span>Target Department: <strong className="text-white uppercase">{successPrompt.department}</strong></span>
                        <span>Priority: <strong className="text-amber-400 capitalize">{successPrompt.priority || "Normal"}</strong></span>
                      </div>
                    )}
                  </div>
                )}
              ...
              <div className="flex items-center justify-end gap-3 pt-2 border-t border-slate-800/80">
                <button
                  type="button"
                  onClick={() => setSuccessPrompt(null)}
                  className="w-full sm:w-auto px-6 py-2.5 bg-gradient-to-r from-emerald-600 via-teal-600 to-emerald-700 hover:from-emerald-500 hover:to-teal-500 text-white font-bold rounded-2xl text-xs uppercase cursor-pointer shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2"
                >
