...
export const UserGuide: React.FC<UserGuideProps> = ({ onNavigateTab }) => {
  ...
  const handlePrintGuide = async () => {
    ...
    try {
      ...
      await printElement("platform-user-guide", {
        title: "THE_TASSIA_HILL_HOSPITAL_HMS_Operating_Manual_User_Guide",
        paperSize: "a4"
      });
    } catch (err) {
      console.error(err);
  ...
  const handleDownloadGuidePdf = async () => {
    ...
    try {
      ...
      const ok = await downloadElementAsPdf("platform-user-guide", {
        fileName: `The_Tassia_Hill_Hospital_HMS_Platform_User_Guide_${new Date().toISOString().slice(0, 10)}.pdf`,
        title: "The Tassia Hill Hospital HMS Platform User Guide",
        format: "a4",
        scale: 2
      });
      if (ok) {
  ...
  const guideSections: GuideSection[] = [
    ...
    },
    {
      ...
      tips: [
        "All The Tassia Hill Hospital HMS scanner inputs listen automatically for fast hardware keystroke bursts, enabling instantaneous scanning without requiring mouse clicks.",
      ],
    },
    {
      id: "faq",
        ...
        <div className="relative z-10 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
          <div className="space-y-2 max-w-2xl">
            ...
            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-black tracking-tight text-white">
              The Tassia Hill Hospital HMS Platform User Guide
            </h1>
